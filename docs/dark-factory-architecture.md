...
- Shell and `jq` steps establish identity, repository, branch, label, review, check, thread, conflict, and head-SHA facts.
...
- Eligible changes require no human approval based on file category. Workflow, action, infrastructure, authentication, security, permissions, migration, and schema changes use the same automated assurance gates as every other pull request.
- `factory:human-review` is a manually applied emergency stop or explicit exception, not an automatic changed-path classification.
...
| `factory:merge-ready` | Pull request | Deterministic identity, review, check, thread, conflict, and exact-head gates passed | Native auto-merge merges the exact eligible head |
| `factory:human-review` | Issue or pull request | A maintainer manually stopped automation or declared an explicit exception | Maintainer removes the label to resume automated processing |
...
PR Merge Assistant uses the GitHub-rendered Copilot identities `app/copilot-swe-agent` or `Copilot` only to shortlist candidates. Before selection and again in the privileged merge job, it verifies the REST identity is ID `198982749`, login `Copilot`, type `Bot`, the head repository is the current repository, the branch starts with `copilot/`, the base is `main`, and `factory:validating` is present.
...
The controller scans candidate pull requests, computes deterministic actions, drops candidates that are only waiting, and selects exactly one actionable item. Priority is merge-ready, review request, then repair assignment; ties use creation time and pull-request number. A waiting pull request does not block another actionable pull request. Dark Factory Dispatch similarly selects one oldest eligible issue. Neither workflow recursively dispatches follow-up runs.

## Full-scope autonomous eligibility and manual emergency stop

No human approval is required for an otherwise eligible factory pull request. The current Copilot review, green external checks, resolved conversations, conflict-free state, exact-head match, and repository ruleset form the approval chain. The ruleset intentionally sets `required_approving_review_count` to `0`.

Changed paths do not create a separate risk lane. The same merge policy applies when a pull request modifies:

- `.github/workflows/`
- `.github/actions/`
- `infra/`
- Authentication, security, or permissions code
- Database migrations or schemas

The controller never applies `factory:human-review` because of file paths. A maintainer may apply that label manually as an emergency stop or explicit exception. Its presence excludes the pull request during deterministic candidate selection and fails the privileged merge revalidation, regardless of which files changed. Removing the label lets the normal automated gates evaluate the pull request again. Human-authored and Dependabot pull requests remain outside this factory.
...
- **Manual emergency stop:** an explicitly applied `factory:human-review` label blocks candidate selection and final merge revalidation until a maintainer removes it.
...
7. Confirm `factory:human-review` has not been manually applied as an emergency stop.
...
- [PR Merge Assistant run 35952589222](https://github.com/VeVarunSharma/contoso-vibe-engineering/actions/runs/35952589222) completed successfully. Its `enable_pr_automerge` job succeeded while the other transition jobs were skipped.
...
This historical example proves the no-human-approval assurance chain for a documentation change. Under the current full-scope policy, the same deterministic identity, review, external-check, thread, conflict, exact-head, native auto-merge, and main-ruleset gates apply to every eligible path category.
