  ...
  for (const file of files) {
    try {
      ...
      if (review.verdict === "error") {
        console.warn(
          `⚠️  ${file}: model failed (verdict: error) — excluded from consensus vote`,
        );
      ...
      if (
        ...
        console.warn(
          `⚠️  Skipping ${file}: invalid verdict "${review.verdict}"`,
        );
...
function computeConsensus(validReviews, errorReviews) {
  ...
  const threshold =
    THRESHOLD_MAP[total] ?? Math.ceil(total * APPROVAL_THRESHOLD_RATIO);
...
function findingKey(f) {
  const descSnippet = (f.description ?? "")
    .slice(0, 120)
    .toLowerCase()
    .replace(/\s+/g, " ");
...
function generateMarkdownReport(reviews, errorReviews, consensus, findings) {
  ...
  if (consensus.verdict === "approve") {
    ...
    lines.push(
      `## Consensus: ⚠️ ERROR — insufficient valid reviews (${consensus.total}/${MIN_QUORUM} quorum)\n`,
    );
  }
  ...
  for (const r of reviews) {
    ...
    const verdictLabel =
      r.verdict === "approve" ? "Approve" : "Request Changes";
  ...
  if (consensus.errored > 0) {
    if (consensus.verdict === "error") {
      lines.push(
        `> ⚠️ **${consensus.errored}** model(s) failed and were excluded. At least ${MIN_QUORUM} valid reviews are required.\n`,
      );
    } else {
      lines.push(
  ...
  if (findings.crossModel.length > 0) {
    ...
    lines.push(
      "These issues were independently identified by multiple models — highest priority.\n",
    );
  ...
  for (const r of reviews) {
    ...
    lines.push(`<details>`);
    lines.push(
      `<summary>${emoji} <strong>${model}</strong> — ${r.summary ?? "No summary"}</summary>\n`,
    );
    ...
    if ((r.findings ?? []).length > 0) {
      ...
      for (const f of r.findings) {
        const loc =
          f.line != null ? `${f.file}:${f.line}` : (f.file ?? "general");
        lines.push(
          `- ${severityEmoji(f.severity)} **${f.severity}** (${f.category}) — \`${loc}\``,
        );
        lines.push(`  ${f.description}`);
        if (f.suggestion) {
          lines.push(`  > 💡 ${f.suggestion}`);
  ...
  if (consensus.verdict === "error") {
    lines.push(
      `*Received ${totalModels} AI model result(s) via GitHub Copilot CLI • Minimum quorum: ${MIN_QUORUM} valid reviews*`,
    );
  } else {
    lines.push(
...
function main() {
  ...
  if (consensus.verdict === "approve") {
    ...
    process.exit(0);
  } else if (consensus.verdict === "error") {
    console.error("\n❌ Consensus: ERROR — insufficient valid reviews.");
    process.exit(1);
  } else {
